...
def _compute_hospital_coverage(
    ...
    Returns list of dicts — one per actionable hospital — with:
        ...
        coverage_status: ALLOCATED | PARTIAL | NONE | COVERED_BY_FACTORY
    """
    ...
    for h in pkg.hospitals:
        ...
        result.append({
            ...
            "prophet_forecast_30d": round(h.prophet_forecast_30d),
        })
...
def _is_fully_covered(coverage: list) -> bool:
    ...
    """
    return all(
        h["coverage_status"] in ("ALLOCATED", "COVERED_BY_FACTORY")
        for h in coverage
    )
...
def _build_review_package(
    ...
    procurement_section = {
        ...
        "api_error":         procurement.get("api_error", False),
        "api_error_msg":     procurement.get("api_error_msg"),
    }
...
def aggregate(
    ...
    for pkg in packages:
        ...
        if verbose:
            ...
            full    = sum(1 for h in cov if h["coverage_status"] in ("ALLOCATED","COVERED_BY_FACTORY"))
