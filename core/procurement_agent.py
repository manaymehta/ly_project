...
def _execute_order(
    ...
    for hid in sorted_hids:
        ...
        for did in dist_list:
            ...
            # Always clamp to still_needed so a bad cap can never over-allocate beyond
            # what the hospital actually requires (safety net for Bug 2).
            still_needed = max(0, remaining_need)
            if unit_caps and hid in unit_caps and did in unit_caps[hid]:
                units_from_did = min(unit_caps[hid][did], avail, still_needed)   # cap from LLM scratchpad + safety clamp
            else:
...
def _call_gemini(prompt: str, max_tokens: int = 2048, _retries: int = 2) -> str:
    ...
    for attempt in range(_retries + 1):
        try:
            ...
            last_exc = e
            err_str  = str(e)
            # Retry on transient 500 server errors — not on 429 (rate limit already
            # has its own backoff) or 4xx client errors
            is_500   = "500" in err_str and "INTERNAL" in err_str.upper()
            if is_500 and attempt < _retries:
                wait = 4 * (attempt + 1)
                ts   = datetime.now().strftime("%H:%M:%S")
                print(
                    f"  [{ts}] [{thread_name}] 500 INTERNAL — retry {attempt+1}/{_retries} in {wait}s",
...
def run_procurement_agent(
    ...
    except Exception as _call1_exc:
        if verbose:
            print(f"    [Call 1 FAILED: {type(_call1_exc).__name__}] — marking api_error for retry")
        with open(_raw_log, "w", encoding="utf-8") as _fp:
            _fp.write(f"=== CALL 1: FAILED ({type(_call1_exc).__name__}: {_call1_exc}) ===\n")
        demand_override = {h["hospital_id"]: h.get("bridge_units_needed", 0) for h in gap_hospitals}
        result.update({
            "parse_ok":               False,
            "api_error":              True,
            "api_error_msg":          str(_call1_exc),
            "call_count":             1,
            "bridge_order":           None,
            "recommendation_summary": f"LLM API error — procurement not computed. Retry required.",
            "is_dicey_case":          False,
            "option_a":               [],
            "option_b":               None,
            "total_stock_gap":        sum(h.get("bridge_units_needed", 0) for h in gap_hospitals),
            "procurement_viable":     False,
            "caveats":                [f"LLM API error on Call 1: {_call1_exc}. Use the Retry button to re-run."],
        })
        return result
    ...
    # Retries up to 2 times with exponential backoff (4s, 8s) — same as Call 1.
    # If all retries fail, return api_error so the dashboard shows a Retry button.
    ...
    if a_scratchpad or b_scratchpad:
        ...
        _CALL2_RETRIES = 2
        _call2_last_exc = None
        _call2_succeeded = False
        for _attempt in range(_CALL2_RETRIES + 1):
            try:
                ...
                extracted, ok_e, _err_e = _parse_json(raw_extract)
                if ok_e:
                    ...
                    _call2_succeeded = True
                    break
                else:
                    # Parse error — treat as a retryable failure
                    _call2_last_exc = Exception(f"Call 2 JSON parse failed: {_err_e}")
                    if _attempt < _CALL2_RETRIES:
                        import sys as _sys, time as _time2
                        from datetime import datetime as _dt
                        _wait = 4 * (_attempt + 1)
                        _ts   = _dt.now().strftime("%H:%M:%S")
                        print(
                            f"  [{_ts}] Call 2 parse error — retry {_attempt+1}/{_CALL2_RETRIES} in {_wait}s",
                ...
                if _attempt < _CALL2_RETRIES:
                    ...
                    _ts   = _dt.now().strftime("%H:%M:%S")
                    print(
                        f"  [{_ts}] Call 2 {type(_e2).__name__} — retry {_attempt+1}/{_CALL2_RETRIES} in {_wait}s",
                        file=_sys.stderr, flush=True,
                    )
                    _time2.sleep(_wait)
                continue

        if not _call2_succeeded:
            with open(_raw_log, "a", encoding="utf-8") as _fp:
                _fp.write(f"\n\n=== CALL 2: ALL RETRIES FAILED ({_call2_last_exc}) ===\n")
            if verbose:
                print(f"    [Call 2 FAILED after {_CALL2_RETRIES} retries: {_call2_last_exc}] — marking api_error for retry")
            result.update({
                "parse_ok":               False,
                "api_error":              True,
                "error_stage":            "call_2_extraction",
                "api_error_msg":          str(_call2_last_exc),
                "call_count":             2,
                "bridge_order":           None,
                "recommendation_summary": "LLM API error on scratchpad extraction — procurement not computed. Retry required.",
                "is_dicey_case":          False,
                "option_a":               [],
                "option_b":               None,
                "total_stock_gap":        sum(h.get("bridge_units_needed", 0) for h in gap_hospitals),
                "procurement_viable":     False,
                "caveats":                [f"LLM API error on Call 2 (scratchpad extraction): {_call2_last_exc}. Use the Retry button to re-run."],
            })
            return result
