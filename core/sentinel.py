...
def _check_taxonomy(node_type: str, event_type: str, severity: str) -> tuple[bool, list]:
    ...
    Hard-fails (raises SentinelError) if event_type is not valid for node_type
    at any severity — this is an invalid combination that must not flow through.

    Soft-warns (returns False + warning) if event_type is valid for node_type
    but the specific severity is not in the taxonomy — severity fallback is
    ...
    valid_events_for_node = sorted({k[1] for k in DISRUPTION_TAXONOMY if k[0] == node_type})
    partial_match = event_type in valid_events_for_node

    if not partial_match:
        # Hard fail — event_type is entirely unknown for this node_type
        raise SentinelError(
            f"Event type '{event_type}' is not valid for node type '{node_type}'. "
            f"Valid types: {', '.join(valid_events_for_node)}"
        )

    # event_type exists for node_type but not at this severity — soft warning only
    warnings.append(
